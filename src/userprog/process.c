...
bool load(const char *file_name, void (**eip)(void), void **esp)
{
  ...
  char *remain_ptr;
  char *temp_file_name;
  strlcpy(input_line, file_name, 100);
  temp_file_name = strtok_r(input_line, " ", &remain_ptr);
  char *token = temp_file_name;
  while (true)
  {
    if (token == NULL)
    {
    ...
    token = strtok_r(NULL, " ", &remain_ptr);
    argc++;
  }

  char *argv[argc];
  strlcpy(input_line, file_name, 100);
  argv[0] = strtok_r(input_line, " ", &remain_ptr);
  token = temp_file_name;
  int count = 0;
  while (true)
  {
    if (token == NULL)
